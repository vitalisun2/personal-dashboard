...
<template>
  <section class="planning-page" aria-labelledby="planning-heading">
    <div class="planning-heading"><div><p class="eyebrow">Видеть перспективу</p><h1 id="planning-heading">Планирование</h1></div><button class="planning-primary" @click="startCreate('project')">＋ Новый проект</button></div>
    ...
    <div class="planning-toolbar"><div class="project-picker"><label for="project-picker">Проект</label><select id="project-picker" :value="project?.id || ''" @change="goProject(($event.target as HTMLSelectElement).value)"><option v-for="item in state.projects.filter(p => state.showArchived || !p.isArchived)" :key="item.id" :value="item.id">{{ item.title }}{{ item.isArchived ? ' · архив' : '' }}</option></select></div><button class="planning-quiet" @click="state.showArchived = !state.showArchived">{{ state.showArchived ? 'Скрыть архив' : 'Показать архив' }}</button><button v-if="project" class="planning-quiet" @click="startEdit('project', project)">Изменить проект</button><button v-if="project" class="planning-quiet" @click="remove('project', project)">Удалить</button><button v-if="project && !project.isArchived" class="planning-quiet" @click="setProjectArchived(true)">В архив</button><button v-if="project?.isArchived" class="planning-quiet" @click="setProjectArchived(false)">Восстановить</button></div>

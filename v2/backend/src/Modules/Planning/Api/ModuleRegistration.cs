...
public static class PlanningApiModule
{
    public static IEndpointRouteBuilder MapPlanningApi(this IEndpointRouteBuilder endpoints)
    {
        ...
        group.MapDelete("/projects/{id:guid}", async (Guid id, VersionInput input, PlanningService service, CancellationToken ct) => { await service.DeleteProjectAsync(id, input.ExpectedVersion, ct); return Results.NoContent(); });
        ...
        group.MapDelete("/projects/{id:guid}/milestones/{milestoneId:guid}", (Guid id, Guid milestoneId, VersionInput input, PlanningService service, CancellationToken ct) => service.DeleteMilestoneAsync(id, milestoneId, input.ExpectedVersion, ct));
        ...
        group.MapDelete("/projects/{id:guid}/milestones/{milestoneId:guid}/features/{featureId:guid}", (Guid id, Guid milestoneId, Guid featureId, VersionInput input, PlanningService service, CancellationToken ct) => service.DeleteFeatureAsync(id, milestoneId, featureId, input.ExpectedVersion, ct));
        return endpoints;
    }
